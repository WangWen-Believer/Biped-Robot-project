...
void ROBOT_output(void)
{
   ...
   *  Sum: '<S11>/Subtract'
   */
  ROBOT_B.Gain_n = (ROBOT_P.Constant22_Value + ROBOT_P.Constant_Value_bj) *
  ...
  diagnosticManager = (NeuDiagnosticManager *)ROBOT_DW.STATE_1_DiagnosticManager;
...
void ROBOT_update(void)
{
...

  {
    ...
    ROBOT_M->Timing.t[1] = ROBOT_M->Timing.clockTick1 *
      ROBOT_M->Timing.stepSize1 + ROBOT_M->Timing.clockTickH1 *
      ROBOT_M->Timing.stepSize1 * 4294967296.0;
  }
}
...
{
  {
    ...
    modelParameters_mFixedStepSize = 0.2;
    ...
    expl_temp.mRTWModifiedTimeStamp = 4.39589247E+8;
...
extern "C" RT_MODEL_ROBOT_T *ROBOT(void)
{
  /* Registration code */
  ...
  {
    int_T *mdlTsMap = ROBOT_M->Timing.sampleTimeTaskIDArray;
    mdlTsMap[0] = 0;
    mdlTsMap[1] = 1;
    ROBOT_M->Timing.sampleTimeTaskIDPtr = (&mdlTsMap[0]);
    ROBOT_M->Timing.sampleTimes = (&ROBOT_M->Timing.sampleTimesArray[0]);
    ROBOT_M->Timing.offsetTimes = (&ROBOT_M->Timing.offsetTimesArray[0]);

    /* task periods */
    ROBOT_M->Timing.sampleTimes[0] = (0.0);
    ROBOT_M->Timing.sampleTimes[1] = (0.2);

    /* task offsets */
    ROBOT_M->Timing.offsetTimes[0] = (0.0);
    ROBOT_M->Timing.offsetTimes[1] = (0.0);
  }

  rtmSetTPtr(ROBOT_M, &ROBOT_M->Timing.tArray[0]);

  {
    int_T *mdlSampleHits = ROBOT_M->Timing.sampleHitArray;
    mdlSampleHits[0] = 1;
    mdlSampleHits[1] = 1;
    ROBOT_M->Timing.sampleHits = (&mdlSampleHits[0]);
  }

  rtmSetTFinal(ROBOT_M, -1);
  ROBOT_M->Timing.stepSize0 = 0.2;
  ROBOT_M->Timing.stepSize1 = 0.2;

  /* External mode info */
  ROBOT_M->Sizes.checksums[0] = (2231954234U);
  ROBOT_M->Sizes.checksums[1] = (36592174U);
  ROBOT_M->Sizes.checksums[2] = (4294489277U);
  ROBOT_M->Sizes.checksums[3] = (1344945145U);
  ...
  ROBOT_M->solverInfoPtr = (&ROBOT_M->solverInfo);
  ROBOT_M->Timing.stepSize = (0.2);
  rtsiSetFixedStepSize(&ROBOT_M->solverInfo, 0.2);
  rtsiSetSolverMode(&ROBOT_M->solverInfo, SOLVER_MODE_SINGLETASKING);

  /* block I/O */
  ROBOT_M->blockIO = ((void *) &ROBOT_B);
  (void) memset(((void *) &ROBOT_B), 0,
                sizeof(B_ROBOT_T));

  /* parameters */
  ROBOT_M->defaultParam = ((real_T *)&ROBOT_P);

  /* states (dwork) */
  ROBOT_M->dwork = ((void *) &ROBOT_DW);
  ...
  /* Initialize Sizes */
  ROBOT_M->Sizes.numContStates = (0);  /* Number of continuous states */
  ROBOT_M->Sizes.numY = (0);           /* Number of model outputs */
  ROBOT_M->Sizes.numU = (0);           /* Number of model inputs */
  ROBOT_M->Sizes.sysDirFeedThru = (0); /* The model is not direct feedthrough */
  ROBOT_M->Sizes.numSampTimes = (2);   /* Number of sample times */
  ROBOT_M->Sizes.numBlocks = (979);    /* Number of blocks */
  ROBOT_M->Sizes.numBlockIO = (46);    /* Number of block outputs */
  ROBOT_M->Sizes.numBlockPrms = (69);  /* Sum of parameter "widths" */
  return ROBOT_M;
}

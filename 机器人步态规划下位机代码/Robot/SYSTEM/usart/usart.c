...
u8 Matlab_data[21]={0};// matlab ·ÂÕæÊý¾Ý
u8 angle_7[20]={0};
u8 angle_8[20]={0};  
////////////////////////////////////////////////////////////////////////////////// 	 
...
{		
	...
	UART1_Handler.Init.BaudRate=128000;				    //²¨ÌØÂÊ
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance==USART2)//Èç¹ûÊÇ´®¿Ú2£¬½øÐÐ´®¿Ú2 MSP³õÊ¼»¯
	{
	 ...
	 __HAL_UART_ENABLE_IT(huart,UART_IT_IDLE);		//¿ªÆô½ÓÊÕÖÐ¶
		...
		if(huart->Instance==USART1)//Èç¹ûÊÇ´®¿Ú2£¬½øÐÐ´®¿Ú2 MSP³õÊ¼»¯
		{
		
		__HAL_RCC_GPIOA_CLK_ENABLE();
		__HAL_RCC_USART1_CLK_ENABLE();			//Ê¹ÄÜUSART3Ê±ÖÓ
	  
		GPIO_Initure.Pin=GPIO_PIN_10;			//PB10
		GPIO_Initure.Mode=GPIO_MODE_AF_PP;		//ÍÆÍìÊä³ö
		GPIO_Initure.Pull=GPIO_PULLUP;			//ÉÏÀ­
		GPIO_Initure.Speed=GPIO_SPEED_FAST;		//¸ßËÙ
		GPIO_Initure.Alternate=GPIO_AF7_USART1;	//¸´ÓÃÎªUSART3
	 ...
	 __HAL_UART_ENABLE_IT(huart,UART_IT_IDLE);		//¿ªÆô½ÓÊÕÖÐ¶
		 __HAL_UART_ENABLE_IT(huart,UART_IT_RXNE);		//¿ªÆô½ÓÊÕÖÐ¶ÏÏ
		HAL_NVIC_EnableIRQ(USART1_IRQn);				//Ê¹ÄÜUSART3ÖÐ¶ÏÍ¨µÀ
...
{  
	...
	u8 *matlab_data=&res;
  	...
  	OSIntEnter();    
#endif
if((__HAL_UART_GET_FLAG(&UART1_Handler,UART_FLAG_RXNE)!=RESET))  //¿ÕÏÐÖÐ¶Ï	
{
...
if((__HAL_UART_GET_FLAG(&UART1_Handler,UART_FLAG_IDLE)!=RESET))  //¿ÕÏÐÖÐ¶Ï
{
   	Matlab_cnt=0;
	  __HAL_UART_CLEAR_IT(&UART1_Handler, UART_CLEAR_IDLEF);//Çå³ýÖÐ¶Ï±êÖ¾Î»
//	  Check_Receivedata(&Receive_data[0],&Servo_State[0][0]);                                 //½«²éÑ¯»ØÀ´µÄ¶æ»úµÄ²ÎÊý·ÅÈë¶æ»úpositionÖÐ
}
